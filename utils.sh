...
get_prebuilts() {
	...
	for src_ver in "$cli_src CLI $cli_ver cli" "$patches_src Patches $patches_ver patches"; do
		...
		if [ "$ver" = "latest" ]; then
			if [ "$is_gitlab" = true ]; then rv_rel+="/permalink/latest"
			else rv_rel+="/latest"; fi
			name_ver="*"
		else
			rv_rel+="/tags/${ver}"
			name_ver="$ver"
		...
		if [ -z "$file" ]; then
			...
			if [ "$is_gitlab" = true ]; then
				matches=$(jq -e '[.assets.links[] | select(.name | (endswith("asc") or endswith("json")) | not)]' <<<"$resp") || return 1
			else
			...
			if [ "$is_gitlab" = true ]; then
				url=$(jq -r .direct_asset_url <<<"$asset")
			else
				url=$(jq -r .url <<<"$asset")
			fi
			name=$(jq -r .name <<<"$asset")
			file="${dir}/${name}"
			if [ "$is_gitlab" = true ]; then gl_dl "$file" "$url" >&2 || return 1
			else gh_dl "$file" "$url" >&2 || return 1; fi
...
gl_or_gh_req() {
	...
	if [ "$is_gitlab" = true ]; then
		if [ "$GL_HEADER" ]; then _req "$url" "$out" -H "$GL_HEADER"
		else _req "$url" "$out"; fi
	else
		gh_req "$url" "$out"
	fi
}
...
gl_dl() {
	if [ ! -f "$1" ]; then
		pr "Getting '$1' from '$2'"
		if [ "$GL_HEADER" ]; then _req "$2" "$1" -H "$GL_HEADER"
		else _req "$2" "$1"; fi
	fi
}
